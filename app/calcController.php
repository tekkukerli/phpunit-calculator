...
if (isset($_POST['submit'])) {
    ...
    if (is_numeric($_POST['number1']) && is_numeric($_POST['number2'])) {
        ...
        } else if ($_POST['operation'] == 'percentage') {
            $total = $calculator->percentage($_POST['number1'], $_POST['number2']);
        }

        if($_POST['operation'] == 'percentage') {
            echo "<h1>{$_POST['number1']} % of {$_POST['number2']} equals {$total}</h1>";
        } else {
